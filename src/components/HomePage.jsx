    ...
    <div className="min-h-screen bg-amber-100 flex flex-col items-center justify-center">
      {slides[currentIndex]?.type === "image" ? (
        ...
        <div className="h-screen w-full flex flex-col items-center relative">
          <h1 className="sm:text-9xl md:text-5xl font-extrabold text-blue-800 absolute top-8">
          ...
          <div className="flex-grow flex justify-center items-center w-full">
            ...
            >
              {slides[currentIndex]?.activities.length === 0 ? (
                <div className="sm:text-6xl md:text-3xl  text-gray-700 font-bold text-center col-span-full">
                  No Activity for now
                </div>
              ) : (
                slides[currentIndex]?.activities.map((activity) => (
                  ...
                  >
                    <h2 className="sm:text-8xl md:text-2xl font-semibold text-center">
                      {activity.name}
                    </h2>
                    <p className="sm:text-6xl md:text-2xl my-4 font-semibold text-center">
                      {formatDisplayDate(activity.startDate)} -{" "}
                      {formatDisplayDate(activity.endDate)}
                    </p>
                    <p className="sm:text-6xl md:text-2xl font-semibold text-center">
